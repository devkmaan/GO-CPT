  ...
  return (
    <nav className="flex justify-between md:items-center bg-bluePrimary/25 fixed top-0 w-full border border-nav/5 shadow-black/20 shadow-sm px-4 backdrop-blur-md z-[10]">
      ...
      <div className="auth flex gap-4 max-lg:text-xs items-start max-lg:pt-3">
        <Link
          to="/login"
          className="bg-gradient-to-b from-bluePrimary px-1 flex rounded-md border border-white/50 shadow-md shadow-gray-500/50"
        >
          Log In
        </Link>
        <Link
          to="/signup"
          className="bg-gradient-to-t from-bluePrimary px-1 flex rounded-md border border-white/50 shadow-md shadow-gray-500/50"
        >
          Signup
        </Link>
        <div
          className="ham rotate-90 leading-5 md:hidden flex"
          onClick={toggleMenu}
        >
